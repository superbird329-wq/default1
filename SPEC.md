...
3. **My role.** What the owner personally did, stated precisely. This section must distinguish individual contribution from team output. For any collaborative or supervised work, state the supervision honestly: "drafted under the review of a licensed professional engineer" is accurate and is not a weakness. Where a deliverable was built with AI assistance, state that in the same plain way, alongside what the owner decided, specified, and verified himself. An unstated assist is the same failure as an unstated collaborator, and it fails in front of the reader most likely to ask about it.
...
3. **SPT blow-count extraction pipeline (Technical).** The Python tool that extracts standard penetration test data from scanned boring logs, computes N-values, and applies the Caltrans Falsework Manual bearing capacity formula. This is a genuine differentiator. Very few construction management undergraduates build tooling against a technical standard. Frame it around the engineering problem it solves, not around the code. Lead with the fact that refusal conditions must never receive computed N-values, because that detail demonstrates domain judgment rather than programming ability. The tool lives in its own repository, so the intake is a handoff prompt sent to the Claude Code session that has that repository open: `.claude/skills/case-study/prompts/boring-tool-writeup.md`.
